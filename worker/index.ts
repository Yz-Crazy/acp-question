import { isAnswerCorrect, nextReviewDate, normalizeAnswers } from "../shared/quiz";
...
async function login(request: Request, env: Env): Promise<Response> {
  ...
  const account = await env.DB.prepare(
    "SELECT id, username, password_hash, password_salt, role FROM users WHERE username = ? COLLATE NOCASE"
  ).bind(username).first<User & { password_hash: string; password_salt: string }>();
